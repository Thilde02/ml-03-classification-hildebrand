...
def make_plots(
    ...
    LOG.info("Creating chart: actual vs predicted scores")
    ...
    min_score = min(
        comparison_df["Actual Score"].min(),
        comparison_df["Predicted Score"].min(),
    )

    max_score = max(
        comparison_df["Actual Score"].max(),
        comparison_df["Predicted Score"].max(),
    )
