...
def make_clean_view(df: pd.DataFrame) -> pd.DataFrame:
    """Create a cleaned view for classification modeling."""
    ...
    df_clean["pass_fail"] = (df_clean[TARGET_COL] >= 70).astype(int)
...
def train_model(
    ...
    LOG.info(f"Classification accuracy: {accuracy:.2f}")
...
def make_plots(
    ...
    LOG.info("Creating chart: actual vs predicted pass/fail")
    ...
    comparison_df["Actual Label"] = comparison_df["Actual"].map({0: "Fail", 1: "Pass"})

    comparison_df["Predicted Label"] = comparison_df["Predicted"].map(
        {0: "Fail", 1: "Pass"}
    )

    confusion_df = pd.crosstab(
        comparison_df["Actual Label"],
        comparison_df["Predicted Label"],
    )
    ...
    ax.set_title("Confusion Matrix: Actual vs Predicted Pass/Fail")
